/**
 * AARAMBHA Deterministic Risk Engine
 * 
 * Mathematically safe composite risk scoring with 0-100 normalization
 * and dynamic reweighting for missing/unassessed components.
 * 
 * Never lets LLMs fabricate or decide risk scores.
 */
...
interface RiskInputs {
  ...
  bidderCount?: number;
  bidSpread?: number; // % difference between lowest & second lowest
  contractorDelayRate?: number; // %
  contractorCancellationRate?: number; // %
...
export interface RiskComponentBreakdown {
  ...
  baseWeight: number;
  activeWeight: number; // After reweighting for missing components
  contribution: number; // activeWeight * normalizedScore
...
export interface CalculatedRiskResult {
  ...
  assessedWeightRatio: number; // 0.0 to 1.0
...
export function calculateDeterministicRisk(inputs: RiskInputs): CalculatedRiskResult {
  const components: RiskComponentBreakdown[] = [];
  ...
  // Determine Confidence based on assessed weight proportion
...
export interface ProcurementRiskInputs {
  priceDeviationPct?: number;
  progressDisparityPct?: number;
  bidSpreadPct?: number;
  contractorDelayRatePct?: number;
  contractorCancellationRatePct?: number;
  documentMismatchCount?: number;
}

export function calculateProcurementRisk(inputs: ProcurementRiskInputs): CalculatedRiskResult {
  const unitPrice = inputs.priceDeviationPct !== undefined ? 100 + inputs.priceDeviationPct : undefined;
  const medianPrice = inputs.priceDeviationPct !== undefined ? 100 : undefined;
  const financialUtilization = inputs.progressDisparityPct !== undefined ? 50 + inputs.progressDisparityPct : undefined;
  const physicalProgress = inputs.progressDisparityPct !== undefined ? 50 : undefined;
  ...
  return calculateDeterministicRisk({
    ...
    physicalProgress,
    bidderCount: inputs.bidSpreadPct !== undefined ? (inputs.bidSpreadPct < 2.0 ? 3 : 5) : undefined,
    bidSpread: inputs.bidSpreadPct,
    contractorDelayRate: inputs.contractorDelayRatePct,
    contractorCancellationRate: inputs.contractorCancellationRatePct,
    documentMismatchCount: inputs.documentMismatchCount
  });
}
