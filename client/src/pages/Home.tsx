    ...
    <main className="site-shell">
      <header className="site-header">
        ...
        <nav className={`desktop-nav ${menuOpen ? "mobile-nav-open" : ""}`} aria-label="Primary navigation">
          ...
          <a className="nav-cta" href="#explore" onClick={() => setMenuOpen(false)}>
        ...
        <div className="container hero-content">
          <div className="hero-copy">
            ...
            <div className="hero-actions">
              <a href="#explore" className="button button-saffron">Explore platform <ArrowUpRight size={16} /></a>
          ...
          <div className="hero-intelligence-panel" aria-label="Illustrative intelligence preview">
            <div className="panel-kicker"><span><Activity size={14} /> Intelligence preview</span><span className="live-mark"><span /> illustrative</span></div>
            <div className="panel-divider" />
            <div className="panel-heading-row">
              <div><span className="panel-micro">Case signal / 04—17</span><h2>Contract trail needs review</h2></div>
              <span className="risk-badge risk-review">Review</span>
            </div>
            <p className="panel-copy">A linked record shows a cost deviation against comparable works in the same category.</p>
            <div className="signal-bars">
              <div><span>Cost variance</span><b>+18.4%</b></div><div className="bar-track"><span style={{ width: "72%" }} /></div>
              <div><span>Execution delay</span><b>42 days</b></div><div className="bar-track"><span className="bar-saffron" style={{ width: "48%" }} /></div>
            </div>
            <div className="panel-footer"><span><CircleDot size={14} /> 6 linked entities</span><span>Evidence attached <Check size={14} /></span></div>
          </div>
        </div>
      ...
      <section className="proof-strip" aria-label="Illustrative platform coverage">
      ...
      <section className="section section-platform" id="platform">
        <div className="container">
          ...
          <div className="capability-grid">
            <article className="capability-card capability-feature">
            ...
            {capabilities.map((item) => {
              ...
              return <article key={item.number} className={`capability-card capability-${item.tone}`}><div className="capability-top"><span className="card-index">{item.number} / capability</span><span className="card-icon"><Icon size={18} /></span></div><h3>{item.title}</h3><p>{item.copy}</p><div className="tag-row">{item.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><div className="card-arrow"><ArrowUpRight size={16} /></div></article>;
            })}
          </div>
        </div>
      </section>
      ...
      <section className="section engine-section" id="engine">
        <div className="container engine-grid">
          ...
          <div className="engine-copy"><SectionLabel dark>02 / Intelligence engine</SectionLabel><h2>Ask a sharper question.<br /><em>Get the evidence trail.</em></h2><p>AARAMBHA’s AI Investigator helps users interrogate a project, contractor, tender, or risk case in plain language — then points back to the records that support the answer.</p><div className="question-stack"><div className="question-bubble"><Search size={15} /><span>Why was this contract flagged?</span><ArrowUpRight size={14} /></div><div className="question-bubble"><Search size={15} /><span>Which evidence supports this risk score?</span><ArrowUpRight size={14} /></div><div className="question-bubble"><Search size={15} /><span>Show projects with high carried-forward funds.</span><ArrowUpRight size={14} /></div></div><div className="engine-quote"><Bot size={18} /><span>Answers should be explainable enough to act on — and clear enough to challenge.</span></div></div>
        </div>
      </section>
      ...
      <section className="section process-section" id="how-it-works">
        <div className="container">
          ...
          <div className="steps-row">{steps.map(([num, title, copy], index) => <div className="step-item" key={num}><div className="step-number">{num}</div><div className="step-line"><span style={{ width: index === steps.length - 1 ? "0%" : "100%" }} /></div><h3>{title}</h3><p>{copy}</p></div>)}</div>
        </div>
      </section>

      <section className="section risk-section" id="impact">
        ...
        <div className="container anomaly-table-wrap"><div className="table-header"><span>Illustrative signals</span><span>Explainable by design <BadgeCheck size={14} /></span></div>{anomalyRows.map(([type, description, status]) => <div className="anomaly-row" key={type}><span className="anomaly-type">{type}</span><span className="anomaly-description">{description}</span><span className={`risk-badge risk-${status.toLowerCase().replace(" ", "-")}`}>{status}</span><ArrowUpRight size={15} /></div>)}</div>
      </section>

      <section className="section coverage-section" id="coverage">
        <div className="container coverage-grid"><div><SectionLabel>05 / Data transparency</SectionLabel><h2>Trust is built into the way the data is shown.</h2><p>Every insight is grounded in a visible source trail. AARAMBHA makes freshness, reliability, and methodology part of the experience — so users can understand both the finding and its limits.</p><ArrowLink href="#explore">Read the methodology</ArrowLink></div><div className="coverage-card"><div className="coverage-card-head"><img src={markImage} alt="" className="coverage-mark" /><Database size={18} /><span>Source register</span><span className="coverage-status"><span /> Current view</span></div><div className="source-row"><span className="source-icon"><Landmark size={15} /></span><span><b>MPLADS records</b><small>Allocations / works / sanctions</small></span><strong>Verified</strong></div><div className="source-row"><span className="source-icon"><GitBranch size={15} /></span><span><b>Procurement records</b><small>Tenders / awards / contracts</small></span><strong>Linked</strong></div><div className="source-row"><span className="source-icon"><Activity size={15} /></span><span><b>Execution signals</b><small>Progress / payment / completion</small></span><strong>Tracked</strong></div><div className="coverage-card-foot"><span>Last updated / illustrative</span><span>Methodology v0.9 <ArrowUpRight size={13} /></span></div></div></div>
      </section>

      <section className="section explore-section" id="explore">
        <div className="container explore-inner"><div className="explore-mark"><img src={markImage} alt="" /></div><div><SectionLabel dark>06 / Next move</SectionLabel><h2>Make public data easier to question — and easier to trust.</h2><p>Explore how AARAMBHA can help your team move from raw records to earlier, evidence-backed oversight.</p></div><div className="explore-actions"><a href="#top" className="button button-paper">Explore the platform <ArrowUpRight size={16} /></a><a href="#coverage" className="text-action text-action-light">View data coverage <ArrowRight size={16} /></a></div></div>
      </section>

      <footer className="site-footer"><div className="container footer-main"><a className="brand-lockup" href="#top"><img src={markImage} alt="" className="brand-mark" /><span className="brand-wordmark"><strong>AARAMBHA</strong><small>MPLADS intelligence</small></span></a><p>Evidence-led monitoring for public works, procurement, and the people responsible for both.</p><div className="footer-links"><a href="#platform">Platform</a><a href="#engine">Intelligence engine</a><a href="#coverage">Data coverage</a><a href="#how-it-works">Methodology</a></div></div><div className="container footer-bottom"><span>© 2026 AARAMBHA / Concept product for civic intelligence.</span><span>No government affiliation claimed.</span></div></footer>
    </main>
  );
}
